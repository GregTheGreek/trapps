...
struct MenuBarScannerTests {
    ...
    @Test func uniqueTitlesAreUnchanged() {
        let titles = MenuBarScanner.disambiguated([entry("A"), entry("B"), entry("C")]).map(\.title)
        #expect(titles == ["A", "B", "C"])
    }

    @Test func duplicatesGetNumberedSuffixesLeavingFirstBare() {
        let titles = MenuBarScanner.disambiguated([entry("A"), entry("A"), entry("A")]).map(\.title)
        #expect(titles == ["A", "A (2)", "A (3)"])
    }

    @Test func disambiguationIsPerTitle() {
        let titles = MenuBarScanner.disambiguated([entry("A"), entry("B"), entry("A")]).map(\.title)
        #expect(titles == ["A", "B", "A (2)"])
    }

    @Test func emptyInputYieldsEmptyOutput() {
        #expect(MenuBarScanner.disambiguated([]).isEmpty)
    }
}
